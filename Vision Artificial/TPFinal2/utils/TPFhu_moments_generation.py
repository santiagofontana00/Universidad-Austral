...
def write_hu_moments(label, writer):
    files = glob.glob('./TPFinal2/databaseASL/asl_dataset/' + label + '/*')
...
def generate_hu_moments_file():
    ...
    os.makedirs('./TPFinal2/TPFgenerated-files', exist_ok=True)
    
    base_path = './TPFinal2/databaseASL/asl_dataset'
    ...
    with open('./TPFinal2/TPFgenerated-files/TPFhu_moments.csv', 'w', newline='') as file:
