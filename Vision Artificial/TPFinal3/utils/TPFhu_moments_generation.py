...
def write_hu_moments(label, writer):
    files = glob.glob('./TPFinal3/databaseASL/asl_dataset/' + label + '/*')
...
def generate_hu_moments_file():
    ...
    os.makedirs('./TPFinal3/TPFgenerated-files', exist_ok=True)
    
    base_path = './TPFinal3/databaseASL/asl_dataset'
    ...
    with open('./TPFinal3/TPFgenerated-files/TPFhu_moments.csv', 'w', newline='') as file:
